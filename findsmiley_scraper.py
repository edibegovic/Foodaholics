...
def get_query_string(place):
    ...
    if not pd.isnull(address) and address != 'Denmark' and address != 'København':
        try:
            if not pd.isnull(postal_code) and not pd.isnull(city_name):
                address_string = str(int(postal_code)) + ' ' + city_name
            elif not pd.isnull(postal_code) and pd.isnull(city_name):
                address_string = str(int(postal_code))
            elif pd.isnull(postal_code) and not pd.isnull(city_name):
                address_string = city_name
            else:
                return name, None
            street_name = re.search('(?<=^|, )[^,\n]*[A-Z]+.*?([0-9]+.?[A-Z]?)+(?=,)', address).group(0)
            return name, street_name + ', ' + address_string
        except AttributeError:
            return name, address_string
    else:
        return None, None
        ...
        if not pd.isnull(name) and not pd.isnull(address):
            ...
            try:
                ...
                restaurant_list = driver.find_element(By.CLASS_NAME, 'ms-srch-group-content').find_elements(By.NAME,
                                                                                                            "Item")
                best_match_element = ""
                best_match_name_ratio = 0
                best_match_address_ratio = 0
                # get all results for given query string and select that
                # restaurant whose name and address have highest text similarity
                # compared with "name" and "address"
                for entry in restaurant_list:
                    entry_element = entry.find_element(By.CSS_SELECTOR, "div.template_body.row.smiley_row > table > "
                                                                        ...
                                                                        "> div.template_header > h3 > a")
                    entry_name = entry_element.get_attribute('title')
                    entry_name_ratio = fuzz.ratio(name.lower(), entry_name.lower())
                    entry_address = entry.find_element(By.CSS_SELECTOR,
                                                       "#ctl00_ctl41_g_fd204bb3_400b_4d2e_ae04_031a827050ed_csr2_item "
                                                       "> div.template_body.row.smiley_row > table > tbody > "
                                                       "tr.search_row_item > "
                                                       "td.search_col_1.col-lg-4.col-12.smiley_table > div:nth-child("
                                                       "2) > p:nth-child(1)").text + ', ' + entry.find_element(
                        By.CSS_SELECTOR,
                        "#ctl00_ctl41_g_fd204bb3_400b_4d2e_ae04_031a827050ed_csr2_item > "
                        "div.template_body.row.smiley_row > table > tbody > tr.search_row_item > "
                        "td.search_col_1.col-lg-4.col-12.smiley_table > div:nth-child(2) > p:nth-child(2)").text
                    ...
                    # for both name and address of restaurant
                    # (to lower the risk of picking the "wrong" restaurant)
                    if entry_name_ratio > best_match_name_ratio and entry_name_ratio > 50 \
                            and entry_address_ratio > best_match_address_ratio and entry_address_ratio > 50:
                        best_match_element = entry_element
                        best_match_name_ratio = entry_name_ratio
                        best_match_address_ratio = entry_address_ratio
